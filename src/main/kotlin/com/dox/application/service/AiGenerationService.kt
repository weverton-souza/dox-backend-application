...
class AiGenerationService(
    ...
    fun reviewText(command: ReviewTextCommand): AiGenerationResult {
        ...
        try {
            ...
            val formResponses =
                if (!command.formResponseIds.isNullOrEmpty()) {
            ...
            val userPrompt =
                reviewPromptPort.buildUserPrompt(
                    text = command.text,
    ...
    fun validateAndAcquireQuota(
        tenantId: UUID,
        reportId: UUID,
    ): Pair<AiQuota, Semaphore> {
        val quota =
    ...
    fun buildGenerationContext(
        reportId: UUID,
        command: GenerateSectionCommand,
        formResponseIds: List<UUID>? = null,
    ): Pair<String, String> {
        val tenantId = ContextHolder.getTenantIdOrThrow()
        val tenant =
            tenantPersistencePort.findById(tenantId)
        ...
        val hasQuantitativeData =
            !command.quantitativeContext.isNullOrBlank() ||
        ...
        val contextPrompt =
            sectionPromptPort.buildContext(
                customer,
                formResponses,
                template,
                professional,
        ...
        val userPrompt =
            if (!command.previousSections.isNullOrEmpty()) {
                contextPrompt + "\n\n" +
    ...
    fun sanitizeOutput(text: String): String {
        ...
        if (!cleaned.contains("\n")) {
            cleaned =
                cleaned.replace(Regex("([.!?])\\s{2,}([A-ZÀ-Ú])")) {
                    "${it.groupValues[1]}\n\n${it.groupValues[2]}"
                }
        }
