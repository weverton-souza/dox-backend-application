...
class ReportGenerationFacade(
    ...
    override fun generateSection(command: GenerateSectionCommand): AiGenerationResult = aiGenerationService.generateSection(command)

    override fun regenerateSection(command: RegenerateSectionCommand): AiGenerationResult = aiGenerationService.regenerateSection(command)

    override fun reviewText(command: ReviewTextCommand): AiGenerationResult = aiGenerationService.reviewText(command)
    ...
    override fun getUsageSummary(command: GetAiUsageCommand): AiUsageSummary = aiUsageService.getUsageSummary(command)

    override fun getUsageHistory(command: GetAiUsageCommand): List<AiUsage> = aiUsageService.getUsageHistory(command)

    override fun getUsageByReport(reportId: UUID): List<AiUsage> = aiUsageService.getUsageByReport(reportId)

    override fun getQuota(): AiQuota? = aiUsageService.getQuota()

    override fun updateQuota(command: UpdateAiQuotaCommand): AiQuota = aiUsageService.updateQuota(command)

    override fun getAiStatus(): AiStatus = aiUsageService.getAiStatus()

    override fun getGenerationSources(reportId: UUID): List<AiGenerationSource> = aiUsageService.getGenerationSources(reportId)

    override fun getRegenerationInfo(reportId: UUID): RegenerationInfo = aiUsageService.getRegenerationInfo(reportId)
}
