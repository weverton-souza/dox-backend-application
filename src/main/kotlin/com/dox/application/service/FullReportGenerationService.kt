...
class FullReportGenerationService(
    ...
    fun generateFullReport(
        ...
        try {
            ...
            val blocksToGenerate =
                if (command.selectedSections != null) {
            ...
            for ((index, block) in blocksToGenerate.withIndex()) {
                ...
                try {
                    ...
                    val (systemPrompt, userPrompt) =
                        aiGenerationService.buildGenerationContext(
                            command.reportId,
                            sectionCommand,
                    ...
                    val finalUserPrompt =
                        if (sectionPlan.status == "partial" && sectionPlan.warning != null) {
            ...
            if (resolvedFormResponseIds.isNotEmpty() && completedCount > 0) {
                ...
                val sources =
                    resolvedFormResponseIds.mapIndexed { index, responseId ->
                        AiGenerationSource(
                            reportId = command.reportId,
    ...
    private fun buildGenerationPlan(
        ...
        try {
            ...
            val planningPrompt =
                systemPromptPort.buildPlanningPrompt(vertical, titlesText, dataSummary)
    ...
    private fun buildDataSummary(
        formResponse: com.dox.domain.model.FormResponse?,
        command: GenerateFullReportCommand,
    ): String {
    ...
    private fun summarizeForContext(
        sectionType: String,
        text: String,
    ): String {
        val maxChars = 800
