...
class FormServiceImpl(
    ...
    override fun createForm(command: CreateFormCommand): FormWithCurrentVersion {
        ...
        val version = formPersistencePort.saveVersion(
            FormVersion(
                formId = form.id,
                version = 1,
                title = command.title,
    ...
    override fun findAllForms(): List<FormWithCurrentVersion> {
        ...
        return forms.map { form ->
            val version = formPersistencePort.findVersionByFormIdAndVersion(form.id, form.currentVersion)
    ...
    override fun updateForm(command: UpdateFormCommand): FormWithCurrentVersion {
        val form = formPersistencePort.findFormById(command.id)
            ?: throw ResourceNotFoundException("Formulário", command.id.toString())

        val currentVersion = formPersistencePort.findVersionByFormIdAndVersion(form.id, form.currentVersion)
            ?: throw ResourceNotFoundException("Versão do formulário", "${form.id}:v${form.currentVersion}")

        val responseCount = formPersistencePort.countResponsesByFormVersionId(currentVersion.id)

        val updatedForm = formPersistencePort.saveForm(
    ...
    override fun createResponse(command: CreateFormResponseCommand): FormResponse {
        val form = formPersistencePort.findFormById(command.formId)
    ...
    override fun findResponsesByCustomerId(customerId: UUID): List<FormResponse> =
        formPersistencePort.findResponsesByCustomerId(customerId)
