...
class CustomerFilePersistenceAdapter(
    ...
    override fun save(customerFile: CustomerFile): CustomerFile = repository.save(customerFile.toEntity()).toDomain()

    override fun findByCustomerId(customerId: UUID): List<CustomerFile> = repository.findByCustomerIdAndDeletedFalse(customerId).map { it.toDomain() }
    ...
    private fun CustomerFileJpaEntity.toDomain() =
        CustomerFile(
            id = id,
