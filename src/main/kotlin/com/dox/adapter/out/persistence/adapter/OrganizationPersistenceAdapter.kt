...
class OrganizationPersistenceAdapter(
    ...
    override fun findById(id: UUID): Organization? =
        organizationJpaRepository.findById(id).orElse(null)?.toDomain()

    override fun findMembersByUserId(userId: UUID): List<OrganizationMember> =
        memberJpaRepository.findByUserId(userId).map { it.toDomain() }

    override fun findMembersByOrganizationId(organizationId: UUID): List<OrganizationMember> =
        memberJpaRepository.findByOrganizationId(organizationId).map { it.toDomain() }

    override fun saveMember(member: OrganizationMember): OrganizationMember {
        val entity = OrganizationMemberJpaEntity().apply {
            id = member.id
    ...
    override fun existsMember(organizationId: UUID, userId: UUID): Boolean =
        memberJpaRepository.existsByOrganizationIdAndUserId(organizationId, userId)
    ...
    private fun OrganizationMemberJpaEntity.toDomain() = OrganizationMember(
        id = id,
