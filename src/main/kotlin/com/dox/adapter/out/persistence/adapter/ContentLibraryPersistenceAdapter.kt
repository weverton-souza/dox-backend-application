...
class ContentLibraryPersistenceAdapter(
    ...
    override fun findAll(): List<ContentLibraryEntry> =
        repository.findAllByOrderByTypeAscTitleAsc().map { it.toDomain() }

    override fun findByType(type: String): List<ContentLibraryEntry> =
        repository.findByTypeOrderByTitleAsc(type).map { it.toDomain() }

    override fun search(query: String, type: String?): List<ContentLibraryEntry> =
        repository.search(query, type).map { it.toDomain() }

    override fun findById(id: UUID): ContentLibraryEntry? =
        repository.findById(id).orElse(null)?.toDomain()

    override fun save(entry: ContentLibraryEntry): ContentLibraryEntry {
        val entity = ContentLibraryJpaEntity(
    ...
    private fun ContentLibraryJpaEntity.toDomain() = ContentLibraryEntry(
