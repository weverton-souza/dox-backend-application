...
class AiGenerationSourcePersistenceAdapter(
    ...
    override fun findByReportId(reportId: UUID): List<AiGenerationSource> =
        repository.findByReportId(reportId).map { it.toDomain() }

    override fun findByGenerationId(generationId: UUID): List<AiGenerationSource> =
        repository.findByGenerationId(generationId).map { it.toDomain() }

    private fun AiGenerationSource.toEntity() = AiGenerationSourceJpaEntity().apply {
        id = this@toEntity.id
    ...
    private fun AiGenerationSourceJpaEntity.toDomain() = AiGenerationSource(
        id = id,
        reportId = reportId,
