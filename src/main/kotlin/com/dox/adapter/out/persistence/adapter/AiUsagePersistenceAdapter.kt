...
class AiUsagePersistenceAdapter(
    ...
    override fun countByProfessionalAndMonth(professionalId: UUID, month: Int, year: Int): Int {
    ...
    override fun countByReportId(reportId: UUID): Int =
        aiUsageJpaRepository.countByReportIdAndStatusNot(reportId, AiGenerationStatus.ERROR)

    override fun findByProfessionalAndMonth(professionalId: UUID, month: Int, year: Int): List<AiUsage> {
        val (start, end) = monthRange(month, year)
        return aiUsageJpaRepository.findByProfessionalAndPeriod(professionalId, start, end)
            .map { it.toDomain() }
    }

    override fun findByReportId(reportId: UUID): List<AiUsage> =
    ...
    private fun monthRange(month: Int, year: Int): Pair<LocalDateTime, LocalDateTime> {
        val start = LocalDateTime.of(year, month, 1, 0, 0)
        val end = start.plusMonths(1)
        return start to end
    }

    private fun AiUsageJpaEntity.toDomain() = AiUsage(
        id = id,
        reportId = reportId,
